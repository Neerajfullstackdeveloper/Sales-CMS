-- FIXED VERSION: Create facebook_data_comments table WITHOUT foreign key constraint
-- This version works even if facebook_data table doesn't have a primary key
-- Copy and paste this ENTIRE script into Supabase SQL Editor and run it

-- 1. Drop table if exists (to start fresh)
DROP TABLE IF EXISTS public.facebook_data_comments CASCADE;

-- 2. Create the table WITHOUT foreign key constraint to facebook_data
...
-- Note: We're NOT adding a foreign key constraint to facebook_data
-- because the facebook_data table might not have a primary key.
-- Application code will handle the relationship.
...
CREATE POLICY "Users can view facebook data comments"
  ...
  USING (
    ...
    -- Check if facebook_data_shares table exists first
    (
      EXISTS (
...
-- Success message
SELECT '✅ facebook_data_comments table created successfully (without foreign key constraint)!' as result;
